...
def parse_option():
    ...
    # test setting
    parser.add_argument('--val_set', action='store_true', default=False,
                        help='run additional evaluation on validation set')
...
def main():
    ...
    if opt.val_set:
        ...
        print(f'\n {opt.n_shots} shot val_acc: {val_acc:.4f}, val_std: {val_std:.4f}, time: {val_time:.1f}')

        start = time.time()
        val_acc_feat, val_std_feat = meta_test(model, meta_valloader, use_logit=False)
        val_time = time.time() - start
        print(f'\n {opt.n_shots} shot val_acc: {val_acc_feat:.4f}, val_std: {val_std_feat:.4f}, time: {val_time:.1f}')

    # start = time.time()
    # test_acc, test_std = meta_test(model, meta_testloader)
    # test_time = time.time() - start
    # print(f'\n {opt.n_shots} shot test_acc: {test_acc:.4f}, test_std: {test_std:.4f}, time: {test_time:.1f}')

    start = time.time()
    test_acc_feat, test_std_feat = meta_test(model, meta_testloader, use_logit=False)
    test_time = time.time() - start
    print(f'\n {opt.n_shots} shot test_acc: {test_acc_feat:.4f}, test_std: {test_std_feat:.4f}, time: {test_time:.1f}')
