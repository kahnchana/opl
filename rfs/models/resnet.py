...
from util import soft_representation_loss
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, n_blocks, keep_prob=1.0, avg_pool=False, drop_rate=0.0,
                 dropblock_size=5, num_classes=-1, use_se=False, use_norm_inside=False, use_srl=False):
        super(ResNet, self).__init__()

        self.inplanes = 3
        self.use_se = use_se
        self.use_norm_inside = use_norm_inside
        ...
        if use_srl:
            self.srl_weights = torch.eye(num_classes, requires_grad=True)
    ...
    def forward(self, x, labels=None, is_feat=False):
        x = self.layer1(x)
        f0 = x
        x = self.layer2(x)
        f1 = x
        ...
        if self.keep_avg_pool:
            x = self.avgpool(x)
        x = x.view(x.size(0), -1)
        if self.use_norm_inside:
            x = F.normalize(x, p=2, dim=1)
        feat = x
        if self.num_classes > 0:
            x = self.classifier(x)
        if labels is not None and self.use_srl:
            srl_value = soft_representation_loss(feat, labels, self.srl_weights)
            return x, srl_value
        if is_feat:
