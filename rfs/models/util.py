...
def create_model(name, n_cls, dataset='miniImageNet', use_srl=False):
    """create model by name"""
    if dataset in ['miniImageNet', 'tieredImageNet', 'imagenet']:
        ...
        if name.startswith('resnet') or name.startswith('seresnet'):
            model = model_dict[name](avg_pool=True, drop_rate=0.1, dropblock_size=2, num_classes=n_cls, use_srl=use_srl)
        elif name.startswith('convnet'):
            model = model_dict[name](num_classes=n_cls)
        else:
            raise NotImplementedError('model {} not supported in dataset {}:'.format(name, dataset))
